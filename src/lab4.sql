...
-- 3. Добавить к таблице `Orders` не обязательное поле `cipher`, которое должно заполняться автоматически согласно
-- шаблону: `<YYYYMMDD> - <номер_района> - <номер_заказа_в_рамках_месяца>`. Номера не обязательно должны
-- соответствовать дате заказа, если район не известен, то `<номер_района>` равен `0`.
