...
for each row begin
    declare new_order_date int := :new.order_date;
    declare new_delivery_date int := :new.delivery_date;

    if (:new.order_date is null) then
        new_order_date = sysdate;
    end if;

    if (:new.delivery_date is null) then
        new_delivery_date = to_date(new_order_date + 1/24);
    end if;
...
for each row
begin
    declare new_type_change varchar2(10);
    declare last_change date;
